...
function handleError(stats) {
  if (stats.hasErrors() || stats.hasWarnings()) {
    ...
    //process.exit(1) -if running on server, kills it
  }
}
...
module.exports = function (packages, options = {}) {
  ...
  const isCwd = options.cwd || (packages.length === 1 && packages[0].charAt(0) === '.')
  ...
  if (!isCwd) {
    ...
    if (cmd.status !== 0) {
      return Promise.reject(new Error(cmd.stderr.toString()))
    }
  }
